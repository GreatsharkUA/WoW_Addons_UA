...
function LootMaster:OnInitialize()
    ...
    self.RESPONSE = {
        { ["CODE"]      = "NOTANNOUNCED",   ["SORT"] =  100,  ["COLOR"] = {1,1,1},        ["TEXT"] = 'Ще не оголошено кандидату' },
        { ["CODE"]      = "INIT",           ["SORT"] =  200,  ["COLOR"] = {1,0,0},        ["TEXT"] = 'Офлайн або lootmaster не встановлено?' },
        { ["CODE"]      = "WAIT",           ["SORT"] =  300,  ["COLOR"] = {1,0.5,0},      ["TEXT"] = 'Вибір, зачекайте...' },
        { ["CODE"]      = "TIMEOUT",        ["SORT"] =  400,  ["COLOR"] = {1,0,1},        ["TEXT"] = 'Кандидат не відповів вчасно.' },
        { ["CODE"]      = "NEED",           ["SORT"] =  500,  ["COLOR"] = {0.5,1,0.5},    ["TEXT"] = 'Основний спек' },
        { ["CODE"]      = "GREED",          ["SORT"] =  800,  ["COLOR"] = {1,1,0},        ["TEXT"] = 'На продаж' },
        { ["CODE"]      = "DISENCHANT",     ["SORT"] =  900,  ["COLOR"] = {0,0.8,1},      ["TEXT"] = '--Розпилення--' },
        { ["CODE"]      = "PASS",           ["SORT"] = 1000,  ["COLOR"] = {0.6,0.6,0.6},  ["TEXT"] = 'Пропустити' },
        { ["CODE"]      = "AUTOPASS",       ["SORT"] = 1100,  ["COLOR"] = {0.6,0.6,0.6},  ["TEXT"] = 'Автоматичний пропуск (not eligible)' },
        { ["CODE"]      = "OFFSPEC",        ["SORT"] =  700,  ["COLOR"] = {1,1,0.5},      ["TEXT"] = 'Офф спек' },
        { ["CODE"]      = "MINORUPGRADE",   ["SORT"] =  600,  ["COLOR"] = {0.2,0.7,0.2},  ["TEXT"] = 'Незначне покращення' },
    ...
    -- Loot receive types.
    self.LOOTTYPE = {
        { ["CODE"]      = "UNKNOWN",        ["TEXT"] = '%s Отримав %s по невідомій причині%4$s.' },
        { ["CODE"]      = "GP",             ["TEXT"] = '%s Отримав %s за %s GP%s.' },
        { ["CODE"]      = "DISENCHANT",     ["TEXT"] = '%s Отримав %s для розпилення%4$s.' },
        { ["CODE"]      = "BANK",           ["TEXT"] = '%s Отримав %s для Банку%4$s.' }
    }
    ...
    self:Print(format('%s завантажено', version))
end
...
function LootMaster:SlashHandler( input )
  ...
  elseif command=='raidinfo' or command=='ri' or command=='saved' or command=='lock' then
        ...
        if not LootMasterML then
            self:Print('Будьласка увімкніть lootmaster ML модуль')
            return
        end
    ...
    elseif command=='debugtest' then
		...
		self:Print( format('%s завантажено', version) )
        self:Print( 'Цей мод забезпечує повну систему розподілу здобичі для EPGP.' )
        self:Print( 'ВИКОРИСТАННЯ: Створіть групу/рейд, налаштуйте майстра грабіжника. Коли головна здобич випадає, просто клацніть предмет правою кнопкою миші, і інтерфейс користувача відкриється.' )
        self:Print( 'Commandline Tools:' )
        self:Print( '/lm version: Перевірка версії' )
        self:Print( '/lm config: Вікно налаштувань' )
        self:Print( '/lm reset: Скинути налаштування вікна' )
        self:Print( '/lm hide: Вручну сховати Master Looter' )
        self:Print( '/lm show: Вручну відкрити Master Looter' )
        self:Print( '/lm toggle: Вручну перемикайтеся між показом і приховуванням Master Looter UI' )
        self:Print( '/lm add [itemlink]: Вручну додати річ до аддону the Master Looter' )
        self:Print( '/lm announce [itemlink]: Вручну додати річ і оголосити проце групу.' )
